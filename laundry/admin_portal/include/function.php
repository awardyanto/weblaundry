...
{
   $conn_STUDENT = $GLOBALS['db'];
	 $sel="SELECT * from order_detail where Delivery_status !='Diterima' order by  ID  ";
	$info=$conn_STUDENT->query($sel);
...
{
  $conn_STUDENT = $GLOBALS['db'];
  $sel="SELECT * from order_detail where Delivery_status='Diterima' ";
  $info=$conn_STUDENT->query($sel);
  $total=$info->num_rows;
  return $total;
}
...
{
$sel="DELETE FROM user_login WHERE ID ='".$_GET["ID"]."' ";
...
if(isset($_GET["UPsaldo"]))
{
$update="UPDATE user_registration SET ".$amount." WHERE ID ='".$_GET["ID"]."' ";
$objExecute=$db->query($update);
  // if($info){
   if($objExecute)
   {
